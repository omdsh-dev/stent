...
The shared source override also loads `tools/oxlint/stent-plugin.ts` and enables `stent/min-function-lines`. Named functions (declarations, variable-bound functions, and class methods) must occupy at least three effective source lines; blank and comment-only lines are ignored. Anonymous callbacks and object-literal callbacks are intentionally excluded, while named concise arrow functions are checked. Intentional short adapters must use `// oxlint-disable-next-line stent/min-function-lines -- reason` with an explanation.
