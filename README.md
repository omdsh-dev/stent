...
The shared source override also loads `tools/oxlint/stent-plugin.ts` and enables `stent/min-function-lines`. The default policy counts effective source lines, including the function definition line; blank and comment-only lines are ignored. Its baseline thresholds are `declaration: 5`, `expression: 3`, `method: 2`, and `arrow: 3`. The `minimums` option independently configures `declaration`, `expression`, `method`, and `arrow` syntax: an integer sets that syntax's threshold and `false` disables it. Anonymous callbacks and object-literal callbacks are excluded unless explicitly enabled with `includeAnonymous`. Intentional short adapters must use `// oxlint-disable-next-line stent/min-function-lines -- reason` with an explanation.
