...
- **Compat API。** 低层补丁机制的协作式入口。两个面:`observe(name, listener)` 保留面向没有协作式扩展点目标 domain 的观察适配器（目标在 module config 中声明，facade 挂载时动态注册 metadata，对外契约只暴露声明的目标名）。`registerPatch(patch)` / `unregisterPatch` / `disablePatch` / `enablePatch` 打开完整的运行时补丁面——handler 在运行时绑定到 launcher 的 dynamic matcher；该 matcher 会根据插件注册——并带**排他的 id 命名空间**:已被声明的观察目标或更早的注册占用的 id 会失败即显式,且 facade 背后的低层注册表还会拒绝已被其他插件占用的 id,因此排他性跨 facade 实例成立。注册归挂载该 facade 的插件所有;`unregisterPatch` 会禁用**并移除** patch,释放 id 以开始全新的所有权周期。`serveBundle({ route, patches, fallback? })` 暴露运行时浏览器 bundle 原语(`serveBrowserTransform`),bundle 重写也经协作式门面进入。所有面都校验 bridge installation capability(`isStentInstalled`),hooks 缺失时失败即显式。
...
`installStentBootstrap(rows)` 为不使用 launcher 的 embedder 保留兼容入口：在目标模块导入前调用
`installStentHooks()`，patch metadata 和 handler 由插件代码注册。Launcher
preload 通常会更早安装该 matcher。`checkStentRequiredPatches(rows)` 在 boot
后检查 live runtime registry 中的 required entries。二者都从 `stent-dsh`
重新导出。
