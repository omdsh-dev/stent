...
- **Compat API.** The cooperative entry for the low-level patch machinery. Two faces: `observe(name, listener)` keeps the observation adapter for target domains with no cooperative extension point (targets are declared in the module config and their metadata is registered dynamically when the facade mounts; the public contract exposes only the declared target names). `registerPatch(patch)` / `unregisterPatch` / `disablePatch` / `enablePatch` open the full runtime patch surface — handlers are bound at runtime to the launcher's dynamic matcher, which is refreshed from plugin registrations — with an EXCLUSIVE id namespace: an id already claimed by a declared observation target or an earlier registration fails loud, and the low-level registry behind the facade additionally rejects an id owned by a different plugin, so exclusivity holds across facade instances. Registrations are owned by the plugin that mounted the facade; `unregisterPatch` disables AND removes the patch, freeing the id for a fresh ownership cycle. `serveBundle({ route, patches, fallback? })` exposes the runtime browser-bundle primitive (`serveBrowserTransform`) so bundle rewrites also enter through the cooperative facade. All faces verify the bridge installation capability (`isStentInstalled`) and fail loud when the hooks are absent.
...
`installStentBootstrap(rows)` keeps the profile-bootstrap compatibility surface
for embedders that compose without the launcher: it calls `installStentHooks()`
before target imports, and plugin code supplies patch metadata and handlers. The launcher
preload normally installs that matcher earlier. `checkStentRequiredPatches(rows)`
runs after boot and checks required entries in the live runtime registry. Both
are re-exported from `stent-dsh`.
