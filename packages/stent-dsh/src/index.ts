 ...
 * `ctx.stentTools`, `ctx.stentPrompt`, `ctx.stentCommands`), a browser facade
 * (`ctx.stentClient`) that delegates to the authoritative DSH client services,
 * the package invariant companion, and the DSH profile bootstrap that installs
 * the runtime-driven Stent transformation hooks. Patch metadata and handlers
 * are supplied by plugin code through `ctx.stent.register()`; profile rows only
 * control plugin activation. Mount this entry to provide all Host modules;
 * mount a subpath to provide one module.
 *
 ...
 */
async function apply(ctx: Readonly<Context>): Promise<void> {
  await registerCatalogEntries()
