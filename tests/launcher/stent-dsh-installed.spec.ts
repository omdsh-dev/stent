...
writeFileSync(
  join(proj, 'node_modules', '@deepseek-ai', 'dsh-app-boot', 'index.js'),
...
writeFileSync(
  join(proj, 'node_modules', 'js-yaml', 'package.json'),
  JSON.stringify({
    name: 'js-yaml',
    version: '4.0.0',
    main: 'index.js',
...
writeFileSync(
  join(proj, 'node_modules', 'js-yaml', 'index.js'),
  [
...
symlinkSync(
  ...
  join(webProfileDir, 'node_modules', '.bin', 'stent-dsh'),
)
