...
function shimTarget(shimPath: URL): URL | undefined {
  ...
  if (marker !== null && marker[1] !== undefined) {
    return pathToFileURL(resolve(dirname(fileURLToPath(shimPath)), marker[1].trim()))
  }
...
function chaseShim(path: URL): URL {
  ...
  if (fileURLToPath(resolved).endsWith('.js') || (existsSync(resolved) && statSync(resolved).isDirectory()))
    return resolved
  const target = shimTarget(resolved)
  return target !== undefined && existsSync(target) ? pathToFileURL(realpathSync(target)) : resolved
}
...
function normalizeCli(path: URL): URL | undefined {
  let dir = existsSync(path) && statSync(path).isDirectory() ? fileURLToPath(path) : dirname(fileURLToPath(path))
  for (let i = 0; i < 5; i++) {
...
/** Resolve a registry-installed @deepseek-ai/dsh from the caller's project or PATH. */
function resolveInstalledCli(cwd: URL, pathEnv: string | undefined): URL {
  try {
    const pkgJson = createRequire(new URL('package.json', cwd)).resolve('@deepseek-ai/dsh/package.json')
  ...
  console.error('stent-dsh: no DSH path was supplied and no installed @deepseek-ai/dsh was found')
  console.error('  pass --dsh-path <source checkout, package, or bin> or put dsh on PATH')
  process.exit(1)
}
...
export function resolveDshPath(input: URL | undefined, cwd: URL, pathEnv: string | undefined): URL {
  ...
  if (!existsSync(candidate)) {
    console.error(`stent-dsh: DSH path does not exist: ${fileURLToPath(candidate)}`)
    process.exit(1)
  }
  if (statSync(candidate).isDirectory()) {
    const sourceBin = pathToFileURL(join(fileURLToPath(candidate), 'apps/cli/src/bin.ts'))
    if (existsSync(sourceBin)) return pathToFileURL(realpathSync(sourceBin))
    const installedBin = normalizeCli(candidate)
    if (installedBin !== undefined) return pathToFileURL(realpathSync(installedBin))
  }
  if (fileURLToPath(candidate).endsWith('.ts')) return pathToFileURL(realpathSync(candidate))
  const installedBin = normalizeCli(chaseShim(candidate))
  if (installedBin !== undefined) return pathToFileURL(realpathSync(installedBin))
  console.error(`stent-dsh: cannot resolve a DSH CLI from path: ${fileURLToPath(input)}`)
  console.error('  expected a source checkout, @deepseek-ai/dsh package, or bin file')
  process.exit(1)
}
...
export function sourceRootFor(bin: URL): URL | undefined {
  ...
  for (let i = 0; i < 5; i++) {
    if (existsSync(join(dir, 'apps/cli/src/bin.ts'))) return pathToFileURL(dir + sep)
    const parent = dirname(dir)
    if (parent === dir) break
    dir = parent
  }
...
export function resolveHost({ dshPath, pathEnv, cwd }: ResolveOptions): ResolvedHost {
  ...
  if (fileURLToPath(realBin).endsWith('.ts') && sourceRoot === undefined) {
    console.error(`stent-dsh: TypeScript DSH entry is not inside a source checkout: ${fileURLToPath(resolvedDshPath)}`)
    process.exit(1)
  }
  const cliPkgJson =
    sourceRoot === undefined
      ? pathToFileURL(join(dirname(dirname(fileURLToPath(realBin))), 'package.json'))
